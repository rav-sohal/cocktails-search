...
const DrinkDetails = ({id}) => {
  ...
  useEffect(() => {
    axios(`https://www.thecocktaildb.com/api/json/v1/1/lookup.php?i=${query}`)
  ...
  return (
    <React.Fragment>
      ...
      {data.map(drink => (
            ...
            <img className="img-fluid mb-5" style={imgStyle} src={drink.strDrinkThumb} alt="cocktail for the details"/>
            </div>
      ))}
    </React.Fragment>
  );
